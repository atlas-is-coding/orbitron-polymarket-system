...
`github.com/atlasdev/orbitron` — use this exact path in all imports (not `polytrade-bot`).
...
6. **Telegram Bot** (`internal/telegrambot/`) — interactive bot mirroring the TUI. Single-admin model: `cfg.Telegram.AdminChatID` is both notification target and bot controller. `telegrambot.New()` returns `(nil, nil)` when token is empty. `OrderCanceler` interface wraps `TradesMonitor` when trades are enabled. Wire with `startSubsystem("Telegram Bot", ...)` in `main.go`.
   ...
   **Navigation**: `sendOrEdit(chatID, text, keyboard)` edits existing menu message (`BotState.menuMsgID`); `/start` resets to 0.
   **Conversation state**: `BotState.SetPending(input, data)` / `ClearPending()` / `Pending()` — multi-step input (e.g. `"addtrader_addr"` → `"addtrader_label"` → `"addtrader_alloc"`).
   **Trading screen**: `sendTrading(chatID, "orders"|"positions")` — NOT `sendOrders`/`sendPositions` (removed).
   **Settings**: `sectionKeys` map (not `settingsSections`). Specific callbacks like `data == "edit:ui.language"` MUST appear before generic `strings.HasPrefix(data, "edit:")` — Go switch is top-to-bottom. Callback data uses English keys; display labels use `i18n.T().SectionXxx`.
   **i18n**: Use `i18n.T().<Field>` at render-time, never cache. `LanguageChangedMsg` handler in `bot.go` is a no-op.
   **Adding a new string**: add field to `Locale` struct + all 5 `internal/i18n/locales/*.json`.
...
- **New trading strategy**: implement `trading.Strategy` (7 methods: `Name`, `WalletIDs`, `SetWalletIDs`, `SetExecutors`, `Start`, `Stop`, `IsRunning`). `NewEngine(log, WalletFinder)` takes a `WalletFinder` as second arg. Call `engine.Register(s)` before `engine.Start(ctx)`. Use `engine.StartStrategy(ctx, name)` / `engine.StopStrategy(name)` for per-strategy control. `engine.SetStrategyWallets(name, walletIDs, execAdapters)` hot-swaps wallets without stopping the engine.
- **Strategy executor pattern**: each strategy config has `wallet_ids []string` and `execute_orders bool`. Strategies receive executors via `SetExecutors(map[string]interface{})`. Nil executor = signal-only mode (alerts without orders).
- **RiskManager** (`internal/trading/risk/`): `NewManager(cfg.Trading.Risk)`. Call `riskMgr.CanTrade()` before placing orders, `riskMgr.RecordLoss(usd)` after losses. Circuit breaker fires when `DailyLossUSD >= MaxDailyLossUSD`. Reset with `riskMgr.Reset()`.
- **Strategy status**: `trading.GetStrategyRows(engine, WalletProvider)` in `internal/trading/status.go` builds `[]tui.StrategyRow` for TUI. Implement `StrategyDetailsProvider` (`Details() string`) on a strategy to show extra info.
...
Six Group-A strategies in `internal/trading/strategies/` (all implement `trading.Strategy`):

| Strategy | File | Logic | Key func |
|---|---|---|---|
| `ArbitrageStrategy` | `arbitrage.go` | Buys YES+NO when sum < $1.00 | `CheckArbitrageOpportunity(mkt)` |
| `MarketMakingStrategy` | `market_making.go` | Posts bid/ask around midpoint | `CalcMMPrices(mid, spreadPct)` |
| `PositiveEVStrategy` | `positive_ev.go` | Near-resolution mispricing | `CheckPositiveEV(yes, liq, end, ...)` |
| `RisklessRateStrategy` | `riskless_rate.go` | Long-duration low-YES markets | `CheckRisklessRate(yes, end, ...)` |
| `FadeTheChaosStrategy` | `fade_chaos.go` | YES spikes → buy NO | `DetectSpike(prev, curr, threshold)` |
| `CrossMarketStrategy` *(AI-generated)* | `cross_market.go` | negRisk sum ≠ 1.0 | `CalcNegRiskDivergence(prices)` |

All strategies share `strategies.Executor` interface (satisfied by `*copytrading.OrderExecutor`). On signal: fire `tui.StrategyAlertMsg` on EventBus **and** call `notifier.Send()`.

`CooldownTracker` in `fade_chaos.go` is reusable for any per-market dedup.

Config keys: `[trading.risk]` (stop_loss_pct, take_profit_pct, max_daily_loss_usd), `[trading.strategies.arbitrage]`, `[trading.strategies.market_making]`, `[trading.strategies.positive_ev]`, `[trading.strategies.riskless_rate]`, `[trading.strategies.fade_chaos]`, `[trading.strategies.cross_market]` — each has `enabled`, `wallet_ids`, `max_position_usd`, `poll_interval_ms`, `execute_orders`.
...
Key subsections:
- `[trading.risk]` — `stop_loss_pct` (default 20%), `take_profit_pct` (default 50%), `max_daily_loss_usd` (default 100)
- `[trading.strategies.*]` — one section per strategy; each has `enabled`, `wallet_ids`, `execute_orders`, `max_position_usd`, `poll_interval_ms` plus strategy-specific params
...
- **Go HTTP panic → browser "Network Error"**: If a `net/http` handler panics before writing any headers, Go sends a bare 500 with no body then closes the TCP connection. The browser receives a connection reset, which axios reports as `e.message === "Network Error"` (not a parseable JSON error). Always wrap mutation handlers with `recoverMiddleware` in `server.go` so panics return `{"error": "..."}` instead.
...
- **New EventBus messages**: `StrategyAlertMsg{Strategy, Market, Question, Signal, Price, EdgePct, Reason, Executed, OrderID}` — fired by each strategy on signal. `CircuitBreakerMsg{DailyLossUSD, Reason}` — fired by RiskManager. `StrategiesUpdateMsg{Rows []StrategyRow}` — strategy status snapshot. `MarketsUpdatedMsg` now has `Err error` field.
...
- **Polymarket token IDs are decimal**: `clobTokenIds` from Gamma are uint256 decimal strings (digits 0-9 only). ALWAYS parse with `big.Int.SetString(id, 10)` first — never hex-first, because digit-only strings parse as valid hex but with the wrong value, producing a silently incorrect token ID and an invalid order signature.
